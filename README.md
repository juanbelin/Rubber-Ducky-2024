<h1>Rubber Ducky 2024 🦆💻</h1>

<p>A manual Rubber Ducky with an Arduino UNO! 2024 </p>



<p>Transform your Arduino UNO into a <b>Rubber Ducky</b> by reprogramming its microcontroller (ATMega16U2).</p>
<img src="https://github.com/user-attachments/assets/356287e5-751e-4a42-aa80-5a0db8d51b73" alt="Introduccion Image 1" width="300px" />


<p>For the transformation, I used this project: 
  <a href="https://github.com/Lacerda53/duckduino">Duckduino Project</a>.
  I also recommend following this <a href="https://www.youtube.com/watch?v=W6uGHYBAhHQ&ab_channel=s4vitar">video tutorial in Spanish</a>.
</p>

<hr>

<h2>🚀 Necessary Tools</h2>

<ul>
  <li>Arduino UNO</li>
  <li>USB cable</li>
  <li>Computer with Arduino IDE installed</li>
</ul>

<p>Here are some images of the tools you'll need:</p>

<img src="https://github.com/user-attachments/assets/cbcbc2d2-9cce-4cc7-9f73-98a0b72e250b" alt="Tools Image 1" width="300px" />
<img src="https://github.com/user-attachments/assets/8e565b6a-44b8-47c3-a476-8203bc86eac5" alt="Tools Image 2" width="300px" />
<img src="https://github.com/user-attachments/assets/3672de29-0b20-4413-a282-0ab448f036a1" alt="Tools Image 3" width="300px" />
<img src="https://github.com/user-attachments/assets/2a66df40-738b-4400-bb73-2deada6b5c82" alt="Tools Image 4" width="300px" />

<hr>

<h2>⚙️ Transformation Steps</h2>

<ol>
  <li>Clone the repository from GitHub:</li>
  <img src="https://github.com/user-attachments/assets/89ac23ed-2505-45cd-b729-c3b9ef08afde" alt="Clone Repository" width="400px" />

  <li>Provide execution permissions:</li>
  <img src="https://github.com/user-attachments/assets/bf88a06f-2c2c-4f05-86f2-4de70875ee28" alt="Provide Permission" width="400px" />

  <li>Run the script:</li>
  <img src="https://github.com/user-attachments/assets/337c6b71-1731-4c64-8042-7430f910858f" alt="Execute Script" width="400px" />

  <li>Connect the Arduino using two pins:</li>
  <img src="https://github.com/user-attachments/assets/22f0a2a7-7279-4448-a271-d94bccd21827" alt="Connect Pins" width="400px" />

  <li>Disconnect it:</li>
  <img src="https://github.com/user-attachments/assets/2ba20e48-c785-48b8-a558-9ce84dc7a0b1" alt="Disconnect" width="400px" />

  <li>Wait a moment:</li>
  <img src="https://github.com/user-attachments/assets/af8fab7a-924f-4767-8e1f-bf0801cbd87f" alt="Wait" width="400px" />

  <li>Open the Arduino IDE and upload the code:</li>
  <img src="https://github.com/user-attachments/assets/e9d610b5-5e7f-4008-8998-fa0095adc0ff" alt="Upload Code" width="400px" />

  <li>Once uploaded, connect the two pins again:</li>
  <img src="https://github.com/user-attachments/assets/0d273ca3-024c-435a-9bf7-f67f1579a100" alt="Connect Again" width="400px" />

  <li>Remove the ATMega16U2 connected pin:</li>
  <img src="https://github.com/user-attachments/assets/53d35e11-c62d-4d81-a5c9-78da7e49f9f9" alt="Remove Pin" width="400px" />

  <li>Enjoy your Arduino Rubber Ducky! 🎉</li>
  <img src="https://github.com/user-attachments/assets/0822bb8d-681e-417f-ab0d-f927fa7eb585" alt="Enjoy" width="400px" />
</ol>

<hr>

<h2>⚠️ Limitations</h2>

<p>By using Arduino UNO for this transformation, there are some limitations:</p>
<ul>
  <li>Some libraries cannot be used.</li>
  <li>The Arduino programming language (C) is required.</li>
</ul>

<p>However, the <code>&lt;HIDKeyboard.h&gt;</code> library makes it easier to program this kind of payloads in Arduino UNO.</p>

<hr>
<h2>Video</h2>

I leave here the reverse shell video demostration: https://youtu.be/V1sFzoz6uUI?si=7YkpAL0tBI4EBwBv

<p>Also I strongly recommend you check mi presentation to get all the infotmation explained and extra videos.</p>

<h2>💡Presentation</h2>

https://www.canva.com/design/DAGAprMGUgM/pQtOPYVXm4cZ-STU1HUHeA/view?utm_content=DAGAprMGUgM&utm_campaign=designshare&utm_medium=link&utm_source=editor#18
